 ...
 */
package com.google.api.codegen.discovery2.transformer.ruby;
...
public class RubySampleTransformer implements SampleTransformer {
  ...
  private static SampleView createSampleView(
    ...
    RubyNamer namer = new RubyNamer(document);

    List<String> requirePaths = new ArrayList<String>();
    requirePaths.add("googleauth");
    requirePaths.add(namer.getServiceRequirePath());
    builder.requirePaths(requirePaths);

    builder.templateFileName("ruby/discovery_sample.snip");
    builder.outputPath(method.id() + ".frag.rb");
